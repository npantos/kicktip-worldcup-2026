...
def expected_points(matrix, pick_h, pick_a, rules):
    ...
    for h, row in enumerate(matrix):
        for a, p in enumerate(row):
            ...
            if h == pick_h and a == pick_a:
                ep += (draw if pick_h == pick_a else win)["exact"] * p
            elif pick_h == pick_a:
                ep += draw["tendency"] * p  # draws have no goal-diff tier
            elif h - a == pick_h - pick_a:
                ep += win["goal_diff"] * p
            else:
                ep += win["tendency"] * p
    return ep
...
def rank_scorelines(matrix, rules):
    rows = []
    for h in range(MAX_CANDIDATE_GOALS + 1):
        for a in range(MAX_CANDIDATE_GOALS + 1):
            rows.append((expected_points(matrix, h, a, rules), h, a))
...
def main():
    ...
    for pred in day["predictions"]:
        ...
        for name, ph, pd, pa, lams in track_probs(pred):
            ...
            ranked = rank_scorelines(matrix, rules)[: args.top]
